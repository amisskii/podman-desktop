  ...
  test.describe
    .serial('PVC lifecycle test', () => {
      test('Create a new PVC resource', async ({ page }) => {
        await createKubernetesResource(page, KubernetesResources.PVCs, PVC_NAME, PVC_YAML_PATH, PLAY_OPTION);
      ...
      test('Bind the PVC to a pod', async ({ page }) => {
        await createKubernetesResource(page, KubernetesResources.Pods, PVC_POD_NAME, PVC_POD_YAML_PATH, PLAY_OPTION);

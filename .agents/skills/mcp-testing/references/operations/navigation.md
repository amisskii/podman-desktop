...
└── [role="region"][aria-label="Page Name"]
```
...
1. Back link: `click('nav[aria-label="Breadcrumb"] a[aria-label="Back"]')` — goes to previous page
2. Close button: `click('nav[aria-label="Breadcrumb"] button[aria-label="Close"]')` — goes to list page

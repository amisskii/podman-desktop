...
├── button matching /\bAll$/                    ← tab: All
├── button matching /\bCommands$/               ← tab: Commands
├── button matching /\bDocumentation$/          ← tab: Documentation
├── button matching /\bGo to$/                  ← tab: Go to
...
| Element           | Selector                                       |
| ----------------- | ---------------------------------------------- |
| All tab           | button with text matching `/\bAll$/`           |
| Commands tab      | button with text matching `/\bCommands$/`      |
| Documentation tab | button with text matching `/\bDocumentation$/` |
| Go to tab         | button with text matching `/\bGo to$/`         |
...
2. Switch to Go to tab:
   ```
   evaluate('Array.from(document.querySelectorAll("button")).find(b => /\\bGo to$/.test(b.textContent))?.click()')
   ```
...
```
evaluate('Array.from(document.querySelectorAll("button")).find(b => /\\b{TabName}$/.test(b.textContent))?.click()')
```

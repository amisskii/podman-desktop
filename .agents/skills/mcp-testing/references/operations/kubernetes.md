...
| PVCs        | `nav[aria-label="Kubernetes Navigation Bar"] a:has-text("PVCs")`                 |
| ConfigMaps  | `nav[aria-label="Kubernetes Navigation Bar"] a:has-text("ConfigMaps & Secrets")` |
| Contexts    | `nav[aria-label="Kubernetes Navigation Bar"] a:has-text("Contexts")`             |
...
| Delete (row)      | `[role="row"][aria-label="{name}"] button:has-text("Delete")`    |
...
2. Click Contexts: `click('nav[aria-label="Kubernetes Navigation Bar"] a:has-text("Contexts")')`
3. Wait for contexts: `wait('[aria-label="Contexts"]')`
